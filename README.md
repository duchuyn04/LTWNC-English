...
- English Mission: chọn chủ đề, dùng từ trong bộ thẻ để hội thoại thích ứng với gia sư AI, lưu transcript và tổng kết câu cần sửa.
...
English Mission gọi AI qua backend ASP.NET Core. Provider được quản lý tại `/Admin/AiProviders`, hỗ trợ API key tùy chọn, discovery model qua `/models`, kiểm tra kết nối và fallback theo `Priority`. API key được mã hóa bằng ASP.NET Core Data Protection và sau khi lưu chỉ có thể thay mới hoặc xóa.
...
User cấu hình tại `AdminBootstrap:UserId` (ID bất biến của tài khoản, không phải username) được gán role `Admin` khi database đã áp đủ migration. Có thể đặt bằng biến môi trường `AdminBootstrap__UserId`. Sau khi được gán role, user cần đăng xuất và đăng nhập lại để cookie nhận role claim.

Provider từ xa bắt buộc HTTPS; HTTP chỉ được phép cho localhost/loopback. Nếu timeout, lỗi mạng, HTTP 429/5xx hoặc output AI sai schema, router thử provider tiếp theo. HTTP 400/401/403 được coi là lỗi cấu hình và không bị che bởi fallback.
...
- Trang không tồn tại trả HTTP `404` và hiển thị giao diện Wrong Turn.
